// deno run --allow-env --allow-read=/etc/alpine-release,$HOME/node_modules test.ts
...
for (const word of wordlists) {
  const seed = `${word} ${word} ${word} ${word} ${word} ${word} ${word} ${word} ${word} ${word} ${word} ${word}`;
  try {
    const identity = Secp256k1KeyIdentity.fromSeedPhrase(seed);
    const principal = identity.getPrincipal();
    console.log(`${word}\t${principal.toText()}`);
  } catch (e) {
    // do nothing
  }
}
